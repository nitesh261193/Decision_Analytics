...
projects_per_contractor_and_month = defaultdict(list)
contractor_project_per_month = defaultdict(list)
        ...
        for contractor in quotes.index:
            ...
            if not pd.isnull(projects.loc[project, month]):
                ...
                if not pd.isnull(quotes.loc[contractor, projects.loc[project, month]]):
                    ...
                    contractor_project_per_month[month].append([project, contractor])

                    quote = quotes.loc[contractor, projects.loc[project, month]]
                    projects_per_contractor_and_month[(contractor, month)].append(project)
        ...
        for contractor in quotes.index:
            if not pd.isnull(projects.loc[project, month]):
                if not pd.isnull(quotes.loc[contractor, projects.loc[project, month]]):
                    ...
                    model.AddBoolOr([eligible_contractor_for_jobs[(project, month, contrac)] for contrac in
        ...
        for contractor in quotes.index:
            if not pd.isnull(projects.loc[project, month]):
                if not pd.isnull(quotes.loc[contractor, projects.loc[project, month]]):
                    ...
                    model.AddBoolOr([eligible_contractor_for_jobs[(p, month, contractor)] for p in
                                     projects_per_contractor_and_month[contractor, month]])
        ...
        for contractor in quotes.index:
            if not pd.isnull(projects.loc[project, month]):
                if not pd.isnull(quotes.loc[contractor, projects.loc[project, month]]):
                    ...
                    model.AddBoolOr([eligible_contractor_for_jobs[(project, m, contractor)] for m in
...
for project in projects.index:
    ...
    if (total_cost_per_project < total_value_per_project):
        ...
        raise TypeError("It shouldn't false")
...
writer = pd.ExcelWriter("DA Ass1 part 3_output.xlsx")
